'''
### Description:
###         Cast pts in gray img when the value is white remove it,else draw it on
###         on gray img,for a purpose of collect sample pts in different connected
###         area.
###         
### Future Goal:
...
if __name__=='__main__':
    ...
    [cv2.circle(t.imgEResized, (pt[1],pt[0]), 1, (0,0,255), 2) for pt in pts]
##    
    t.showImgs()
    
##    # generate 100 random points
##    paths = t.generateRandomPoints(1)
##    # collect all paths of each point
##    pointsAlls = [t.oneToEndGrowth(path) for path in paths]




##    #[[[print(point) for point in points] for points in pointsAll] for pointsAll in pointsAlls]
##
##    # make us a canvas
##    img = np.ones((200, 200))
##    # draw all paths
##    [[[cv2.circle(img, point, 1, (0,255,0), 1) for point in points] for points in pointsAll] for pointsAll in pointsAlls]
##    cv2.imshow('img',img)
##    cv2.waitKey()
