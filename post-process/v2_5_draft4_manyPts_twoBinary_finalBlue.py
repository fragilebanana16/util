...
##        print(euqation(start))
        # overflow switch
        if start < 0 or start > 399: 
##            print("[Info]index overflow",euqation(start) )
            break
        elif euqation(start) >299:
            # y=x to x=y
            pair1 = (int(euqation_(299)),299)
            break
...
##            print("[Info]edge detected")
    ...
    while True:
        start -= 1#
        if start < 0 or euqation(start) < 0 or start > 399: 
##            print("[Info]index overflow2",euqation(start) )
            break
        elif euqation(start) >299:
            pair2 = (int(euqation_(299)),299)
            break
        elif euqation(start) < 0:
...
##    randPt = randPts[0]
    ...
    ##    print("coord1:{}\ncoord2:{}".format(coord1,coord2))
        ...
        euqation1_1 = lambda y:float((y-coord1[0][1])*(coord1[1][0]-coord1[0][0])/\
                                                    (coord1[1][1]-coord1[0][1])+coord1[0][0])
    ...
    ##    print(coord1[0][0],euqation1(coord1[0][0]))
        ...
        euqation2_2 = lambda y:float((y-coord2[0][1])*(coord2[1][0]-coord2[0][0])/\
                                                    (coord2[1][1]-coord2[0][1])+coord2[0][0])
...
##        print(coord11,coord22)
        ...
        euqation22 = lambda x:float((x-coord22[0][0])*(coord22[1][1]-coord22[0][1])/\
                                                    (coord22[1][0]-coord22[0][0])+coord22[0][1])
        # get x
        euqation22_2 = lambda y:float((y-coord22[0][1])*(coord22[1][0]-coord22[0][0])/\
                                                    (coord22[1][1]-coord22[0][1])+coord22[0][0])
        
    ##    print("[Info]2222 this is correction")
    ##    print(coord11[1][0],euqation11(coord11[1][0]))
    ##
    ##    print("[Info]2222 this is correction2")
    ##    print(coord22[1][0],euqation22(coord22[1][0]))
    ##    print("[Info]2222 correction done")
        
        # left growth
        p11,p22 = growth(coord11[0],euqation11,euqation11_1,blue)
        # right growth
        p33,p44 = growth(coord22[0],euqation22,euqation22_2,green)
...
##        [cv2.circle(imgTest,(i[1],i[0]), 4, (150,150,0), -1) for i in [p11,p22,p33,p44]]
        ...
        getItsCoords = whoIsLonger((p11,p22),(p33,p44))
